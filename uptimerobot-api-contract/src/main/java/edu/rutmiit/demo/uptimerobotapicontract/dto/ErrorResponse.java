 ...
 * fieldErrors — список ошибок по отдельным полям, удобно для отображения форм. timestamp + instance
 * — упрощают поиск ошибки в логах.
 */
...
public record ErrorResponse(
        ...
        @Schema(description = "URI-идентификатор типа ошибки. Машиночитаемый; клиент может switch по нему.",
        ...
        @Schema(description = "Детальное описание конкретного случая ошибки", example = "алерт с id=42 не существует")
        String detail,

        @Schema(description = "URI запроса, приведшего к ошибке", example = "/api/alert/42")
        String instance,
        ...
        public record FieldError(
                ...
                @Schema(description = "Причина отклонения", example = "Некорректный код ответа ресурса")
                String message) {
        }
}
